...
explore quantum error correction for nonabelian anyon
...
of error-correction in nonabelian anyon
...
model~\cite{Wootton2013, Hutter2014} that, while nonabelian,
...
In Figure\ref{PseudoCode} we show a pseudo-code listing for this,
...
and error rate per edge $t_{\mbox{sim}}.$
The number of pair-creation
processes accross each edge of the lattice
is sampled from a poisson process,
and $t_{\mbox{sim}}$ indicates the expected value of this number.

The decoder failure rate shows a clear threshold at around 
$t_{\mbox{sim}}=0.124\pm0.002.$ 
Below this threshold errors are increaslingly suppressed 
