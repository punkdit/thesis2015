...
be self-correcting quantum memories.
 ...
 People (physicists) always do this stuff using a spin transform.
 But we can also work with transformations on the group level,
 and later on figure out the spin transform (if needed).
 This is in line with Gottesman's thesis, where states are
 specified by the operators that act on them, instead of
 actually performing operations on states. This is also
 the philosophy of category theory, which tries to lift
...
using the following set of $2n$ independent generators:
$$
\mathrm{JW}_Z :=
\big\{c_{2j}:=Z_1...Z_{j-1} X_j,\ c_{2j+1}:=Z_1...Z_{j-1} Y_j\ \ 
\mbox{for}\ 1\le j\le n\big\}.
$$
That these operators generate $\Pauli_n$ can be seen by considering
the abelianization of $\Pauli_n$ which is the group $\Z_2^{2n}.$
There are two other ways of writing this presentation:
\begin{align*}
\mathrm{JW}_X &:=
\big\{c_{2j}:=X_1...X_{j-1} Y_j,\ c_{2j+1}:=X_1...X_{j-1} Z_j\ \ 
\mbox{for}\ 1\le j\le n\big\},\\
\mathrm{JW}_Y &:=
\big\{c_{2j}:=Y_1...Y_{j-1} X_j,\ c_{2j+1}:=Y_1...X_{j-1} Z_j\ \ 
\mbox{for}\ 1\le j\le n\big\}.
\end{align*}
...
the ground eigenvalue and the first excited state.
These results are obtained by exact diagonalization methods.
}
\end{center}
\end{figure}
...
\begin{figure}\label{PicGapStabs}
\begin{center}
...
\todo{actually the wavefunction becomes
exponentially small but also it is exponentially wide}
