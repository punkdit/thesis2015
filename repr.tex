...
model &\ $n$\ &\ $r$\ &\ $t_X$\ & $|\auto|$ & $|\mathrm{orbits}|$ & Aut(code) \\
\hline
%    1D xy & 4 &  2& 0 & 2 & 3 & 8 \\
%       & 5 &  4& 0 & 10 & 4 & 10 \\
%       & 6 &  4& 0 & 72 & 3 & 12 \\
...
Here we also show Aut(code) which is
the automorphism group of the gauge code.
Elements of this group act by permuting the $n$ qubits.
In detail, such an action is given by three permutation matrices,
$P_n, P_Z, P_X$ such that the following $\Field$-linear equations hold
\begin{align*}
    P_X G_X P_n &= G_X, \\
    P_Z G_Z P_n &= G_Z.
\end{align*}
This implies that $P_n$ commutes with the Hamiltonian
(as $\Complex$-linear operators):
$$
    P_n H = H P_n.
$$
\todo{why does this preserve $H_{0,0}$ ???}
%This group also acts on $H_{0,0}$ 
