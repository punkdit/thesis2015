...
Given a CSS gauge code Hamiltonian $H,$ we see that 
...
irreducible.
\tombstone
%\todo{i don't think we will need this lemma..}
...
And by Lemma 2.2 the top eigenvector of $\Gamma_{t_X}$ is 
...
This idea is also found in the
spectral graph theory literature, see for example Ref.~\cite{Cvetkovic1980}
chapter 5.
...
These are equivelance classes on $V,$ where two vertices $v_1, v_2$ are
deemed equivelant iff there is a graph automorphism that sends $v_1$ to $v_2.$
...
We now show that the apparent arbitrariness in the choice of $D$ does
...
We will see the following lemma demonstrated by numerics in section 2.8.1 and
section 2.8.2 below.
