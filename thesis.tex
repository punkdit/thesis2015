...
We make the restriction that the generators of
of the stabilizer group come from either $\Pauli_n^X$ or $\Pauli_n^Z$.
This is known as a CSS stabilizer code.
...
$$
Using this identification, commutativity of an
operator $u\in\Pauli_n^X$ and $v\in\Pauli_n^Z$
is equivalent to the $\Field^n$ inner product of $u$ and $v$ being zero.
...
We use the rows of $\partial_2^\top$ as $X$ type generators of
the stabilizer group, and the rows of $\partial_1$ as $Z$ type
generators.
...
We now dissect this space $\Field^n$ according to syndrome.
...
\end{array} \right).
\end{align*}
Together this forms the $(L,S,T)$-decomposition of the CSS
stabilizer code.
