...
%%By exploring these codes we are able to 

Chapter 1 introduces the subject of quantum error correction,
from the viewpoint of homology theory. 
These are abelian quantum codes: the check operators
all commute with each other.
When we turn these check operators into the terms of a
Hamiltonian, we can easily find all the eigenvectors and
the spectrum.

The objects of study in chapter 2 are the spectra of 
Hamiltonians built from Pauli spin matrices.
These spin matrices lead a double life:
both as the energetics of a Hamiltonian model, and as
the operators in a quantum error correcting code.
For the Hamiltonian to act as a quantum memory, we
would expect there to be a gap between the groundstate
energy and the first excited state.
In Section 2.10 we present numerical evidence of such a gap,
for the three-dimensional gauge color code Hamiltonian.

The second half of the thesis concerns a quantum
code built using a system supporting anyonic excitations.
These anyons are quasi particles that live in a two
dimensional system and enjoy exotic exchange statistics. 
Understanding these statistics involves an appreciation of
how the three-dimensional world-lines of these particles
get tangled, or braided together. This turns out to be
a Schrodinger picture of the state, and there is a corresponding
Heisenberg picture that involves deforming the underlying
two-dimensional manifold.
These ideas are discussed in Chapter 3, leading up to
...
quantum information is the fusion space of a torus.
